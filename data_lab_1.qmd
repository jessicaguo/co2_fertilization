...
This is a [Quarto document](https://quarto.org/), the successor to Rmarkdown that contains some fun new features, such as the ability to run Python. We will use it in class to combine chunks of R code with blocks of prose. When turning in your assignment, please upload the rendered .pdf to Gradescope/Canvas.

## Part I: Mauna Loa \[CO~2~\]

You have likely seen the Mauna Loa CO~2~ curve presented in lectures and texts. We are going to plot it ourselves! These data were originally sampled by Dr. C. David Keeling of the Scripps Institution of Oceanography and can be downloaded freely from [NOAA](https://gml.noaa.gov/ccgg/trends/data.html). Read more about the history of the Keeling curve [here](https://keelingcurve.ucsd.edu/2013/04/03/the-history-of-the-keeling-curve/).
...
When we get tabular data from online sources, it commonly comes with a header that provides metadata, or information about the dataset. Go ahead and open the .csv file to read the metadata.

### Q1 (0.5 points)

What key pieces of information are conveyed in the metadata? Describe at least 2.
...
Try commenting out each line of the code above with a `#` to understand the purpose of each layer. Add a red line beneath the points to connect the values. Hint: visual layers can be added with `geom_*` or `stat_*`, and you can start typing `?geom_` in the console to see a list of possible functions.
...
For both sets of estimated parameters, there is no standard error on J~max~, due to the fitting method and the fact that light response curves were not measured. Therefore, we will not put much faith in to the J~max~ estimate. Nonetheless, we can use the default plotting method for the `fitaci()` output to see how well the data are matched by the parameters.
...
In this particular case, the fit for *Larix* looks like a standard ACi curve, although *Picea* apparently has the higher V~cmax~. Check in with Fig. 1 from [Dusenge et al. 2024](https://onlinelibrary.wiley.com/doi/full/10.1111/pce.15068). Do our values of V~cmax~ accord with their reported values? Compare your value to the value from Fig. 1.
