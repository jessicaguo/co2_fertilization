...
Finally, your assignment will close with a written discussion synthesizing empirical and theoretical evidence for CO~2~ fertilization of photosynthesis across leaf, ecosystem, and global scales.

### A technical note

This is a [Quarto document](https://quarto.org/), the successor to Rmarkdown that contains some fun new features. We will use it in class to combine chunks of code with blocks of prose. When turning in your assignment, please upload both the .qmd file and the rendered .pdf.

## Part I: Mauna Loa \[CO~2~\]

You have probably seen these data presented in lectures and texts. We are going to plot it ourselves! These data were originally sampled by Dr. C. David Keeling of the Scripps Institution of Oceanography and can be downloaded freely from [NOAA](https://gml.noaa.gov/ccgg/trends/data.html). Read more about the history of the Keeling curve [here](https://keelingcurve.ucsd.edu/2013/04/03/the-history-of-the-keeling-curve/).
...
Let's check this understanding empirically.
...
We will need some additional libraries in this part; use `install.packages()` for the first time only. 
...
plot(pima)
```
In this particular case, the fit for *Larix* looks like a standard ACi curve, although *Picea* apparently has the higher V~cmax~. Check in with Fig. 1 from Dusenge et al. 2024. Do our values of V~cmax~ accord with their reported values?

### Check your understanding (written response required)

The *Larix* ACi curve reports the Ci transition, which can be extracted.
...
A review of FACE experiments is provided as a pdf in this repository. Please read the abstract, introduction, and hypotheses 1, 2, 4, and 8 as evaluated in Norby and Zak (2011). What is some evidence from FACE experiments that CO~2~ fertilization can a negative feedback on anthropogenic carbon emissions by increasing photosynthesis and carbon storage? What are some processes that can disrupt this negative feedback? Please consider the complexity of scaling from the biochemistry of photosynthesis to entire ecosystems.   